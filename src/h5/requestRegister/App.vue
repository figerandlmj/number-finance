<template>
  <div class="registerContainer">
    ...
    <!-- 图形验证码弹框 -->
    <img-code-box v-if="isShowCodeBox" :mobile="telValue" :btnColor="btnColor" :pointCode="'yq'" @isShowVanLoading="isShowVanLoading" @getShotMsgCode="getShotMsgCode" @closeCodeBox="closeCodeBox" @openTipBox="isShowTip"></img-code-box>
    <!-- 打开APP提示框 -->
    <register-tip-box v-if="isShowTipBox" :isPage="isPage" :mobile="telValue" :channelCode="channelCode" :yqUserId="yqUserId" :yqNumber="yqNumber" @closeTipBox="isShowTip"></register-tip-box>
    <!-- vanLoading -->
  ...
  export default {
    ...
    methods:{
      ...
      confirmRegister(){// 立即加入
        ...
        //  事件埋点
        h5EventBuryingPoints('qdzc', 'click_yq_zc', {
          // "mobileNumber": this.telValue,//注册用户手机号
          // "dcqdCode": this.channelCode,
      ...
      register(){// 注册
        ...
        myAxios('/srApis/h5/register', 'get', {
          ...
          this.showVanLoading = false;
          if(json.code === 200) {
            window.location.href = './registerResult.html?mobile=' + this.telValue + '&channel=' + this.channelCode + '&yqUserId=' + this.yqUserId + '&yqNumber=' + this.yqNumber;
      ...
      phoneFocus() {
        //  事件埋点
        h5EventBuryingPoints('qdzc', 'click_yq_sjhsr', {});
      },
      //短信验证码输入框获取焦点事件
      shotMsgCodeFocus() {
        //  事件埋点
        h5EventBuryingPoints('qdzc', 'click_yq_dxyz', {});
      }
    }
  }
</script>
